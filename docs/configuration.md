...
- 비밀값은 env-only입니다. `SLACK_APP_TOKEN`, `SLACK_BOT_TOKEN`, `SENTRY_AUTH_TOKEN`, `GITLAB_TOKEN`은 `.env`나 실행 환경 변수에만 둡니다.
- YAML은 저장소, 브랜치, Slack 채널, runner, GitLab MR 정책처럼 공유 가능한 비밀 없는 값만 담습니다.
...
- MR provider는 GitLab-only입니다. `mr.provider`는 `gitlab`만 지원합니다.
...
| `GITLAB_TOKEN` | 필수 | 없음 | GitLab MR 생성과 branch push에 쓰는 비밀값입니다. env-only로 관리합니다. |
...
| `STATE_DB_PATH` | 선택 | `.omo/state.sqlite` | 로컬 SQLite state와 audit log 파일 경로입니다. 기본값은 작업 디렉터리 기준 상대 경로입니다. |

비밀값 자리에는 실제 로컬 값을 직접 입력해야 합니다. 아래 템플릿은 credential 형태의 예시 문자열을 넣지 않은 안전한 형태입니다.
...
STATE_DB_PATH=.omo/state.sqlite
```
...
`mr.provider`는 MR provider를 고릅니다. 현재 지원값은 `gitlab`뿐입니다.

- `gitlab` 외 provider는 설정 검증에서 거부됩니다.
- GitHub PR provider는 현재 지원 기능이 아니므로 설정하지 않습니다.
