...
이 문서는 현재 구현된 incident-chatops-worker의 런타임 구조를 설명한다. 흐름은 `Sentry polling`으로 감지한 이슈를 Slack 승인으로 처리하고, 로컬 `worktree`에서 runner를 실행한 뒤 필요한 경우 GitLab MR 또는 GitHub PR을 만드는 방식이다.
...
- Repo adapter: 분석 전에는 source repo clean 상태만 확인한다. 수정 job에서만 worker git `worktree`를 열고, 검증을 통과한 수정 branch를 원격으로 `push`한다.
- MR/PR provider: 현재 문서화 대상 구현은 GitLab MR과 GitHub PR 생성 경로다. MR/PR URL은 SQLite에 저장되고 Slack 상태 메시지로 게시된다.
- SQLite state store: incident, job, approval, 분석 요약, verification 요약, MR 링크, incident handoff, Sentry snapshot, audit 데이터를 저장한다.
- MCP handoff server: SQLite에 저장된 handoff만 읽는 token-light MCP stdio surface다. daemon, polling, runner, push, MR/PR 생성을 시작하지 않는다.
...
6. analysis-only 경로는 먼저 incident context를 준비하고 source repo clean 상태를 확인한다. runner는 configured source repo path에서 `analysis_only` 모드로 실행되고, worker git worktree를 만들지 않으며, `worktree.prepare`, branch 생성, push, Git provider MR/PR 생성을 실행하지 않는다. 결과의 구조화된 분석 필드는 SQLite 분석 요약으로 저장된다. runner가 source workspace를 dirty 상태로 만들면 성공으로 보지 않는다.
...
9. fix 경로는 worker git `worktree`를 열고 `worktree.prepare`와 `runners.projectEnv`를 적용한 뒤 runner를 `fix_and_mr` 모드로 실행한다.
10. runner 결과에서 verification summary를 SQLite에 먼저 저장한다. 그 다음 `verification` 결과를 검사하며, 실패하면 `push`와 Git provider MR/PR 생성을 하지 않는다.
11. verification이 통과하면 repo adapter가 branch를 원격으로 `push`한다.
12. push 후 GitLab MR 또는 GitHub PR을 REST API로 생성하고, 링크와 handoff context를 SQLite에 저장한 뒤 workflow를 `mr_created`로 전이한다. handoff는 cleanup 전에 저장된다.
13. Slack thread에는 MR/PR URL, verification 상태, 변경 요약을 포함한 상태 메시지를 게시한다.
14. fix는 `finally` 단계에서 worker worktree cleanup을 시도한다. cleanup 실패는 warning/audit로 남고 이미 저장한 MR/PR 링크와 handoff 결과를 실패로 덮어쓰지 않는다.
...
- `mr_links`: GitLab MR 또는 GitHub PR 생성 성공 시 provider와 URL을 저장한다.
- `incident_handoffs`: Codex App follow-up을 위해 issue id, repo id/path, MR/PR URL, `sourceBranch`, target branch, `headSha`, 분석/변경/검증 요약, readiness, follow-up prompt를 저장한다. raw Sentry payload는 저장하지 않고 summary/prompt는 redaction 후 저장한다.
...
이 시스템의 `trust boundary`는 Slack 사용자 액션, Sentry issue context, runner 출력, Git provider API 응답, 로컬 git 작업 사이에 있다.
...
- Git provider API 응답은 MR/PR URL 저장과 Slack 게시에만 사용한다.
- audit와 사용자-visible Slack 오류는 secret redaction을 거친 메시지를 사용해야 한다.

## Failure State

- 일반 analysis 실패: Sentry context fetch, source repo clean check, runner 실행 같은 단계에서 오류가 나면 workflow는 실패 상태로 전이하고 Slack에 redacted 오류를 게시한다. analysis path는 worker git worktree를 만들지 않는다.
- `verification_failed`: `fix_and_mr` runner 결과의 verification이 실패하면 verification summary는 저장되지만 `push`와 Git provider MR/PR 생성은 실행하지 않는다. Slack에는 verification 실패 상태가 게시된다.
- `mr_failed_after_push`: verification 통과 후 branch `push`는 성공했지만 Git provider MR/PR 생성이 실패한 상태다. audit details에는 branch와 remote가 남고, Slack에는 branch가 retry를 위해 남아 있다는 상태가 게시된다.
...
- 같은 Slack 액션은 `action_idempotency_key`로 deduplicate된다. 이미 처리된 job이면 새 runner, push, Git provider side effect 없이 "already handled" 또는 "already in progress" 메시지를 돌려준다.
...
- MR/PR provider: 현재 운영 문서는 GitLab MR provider와 GitHub PR provider를 기준으로 한다. 둘 다 REST API만 사용하며 provider CLI는 필요하지 않다. 다른 provider를 추가하려면 MR/PR 생성 입력, audit, failure handling이 같은 신뢰 경계와 상태 전이를 지켜야 한다.
